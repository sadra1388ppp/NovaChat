...
public sealed class E2eeDeviceService(AppDbContext db)
{
...
ON DUPLICATE KEY UPDATE
    ...
    {
        await EnsureSchemaAsync(cancellationToken);
        var usernames = await _db.Chats.AsNoTracking().Where(c => c.Id == chatId && !c.IsDeleted).Select(c => c.Members).FirstOrDefaultAsync(cancellationToken);
        if (string.IsNullOrWhiteSpace(usernames)) return [];

        var names = usernames.Split(',', StringSplitOptions.RemoveEmptyEntries | StringSplitOptions.TrimEntries).Distinct(StringComparer.OrdinalIgnoreCase).ToList();
        if (names.Count == 0) return [];

        var memberIds = await _db.Users.AsNoTracking().Where(u => names.Contains(u.Username)).Select(u => u.Id).ToListAsync(cancellationToken);
        if (!memberIds.Contains(userId)) return [];

        var connection = _db.Database.GetDbConnection();
        await using var command = connection.CreateCommand();
        command.CommandText = "SELECT DeviceId, UserId, PublicKeyPem, CreatedAt, LastSeenAt FROM EncryptionDevices WHERE RevokedAt IS NULL AND UserId IN (" + string.Join(',', memberIds.Select((_, i) => "@p" + i)) + ")";
        for (var i = 0; i < memberIds.Count; i++)
        {
            var parameter = command.CreateParameter();
            parameter.ParameterName = "@p" + i;
            parameter.Value = memberIds[i];
        ...
        {
            result.Add(new E2eeDeviceRecord(reader.GetString(0), reader.GetInt64(1), reader.GetString(2), reader.GetDateTime(3), reader.GetDateTime(4)));
        }
        return result;
    }
