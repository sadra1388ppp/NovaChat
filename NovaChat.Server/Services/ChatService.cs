...
public class ChatService
{
    ...
    public async Task<Chat?> CreatePrivateChatAsync(long currentUserId, long otherUserId)
    {
        ...
        try
        {
            var existing = await _context.Chats.Include(c => c.User1).Include(c => c.User2)
                .Where(c => c.Type == ChatType.Private && ((c.User1Id == currentUserId && c.User2Id == otherUserId) || (c.User1Id == otherUserId && c.User2Id == currentUserId)))
                .OrderBy(c => c.Id).ToListAsync();
            if (existing.Count > 0)
            {
                var chat = existing[0];
                if (existing.Count > 1) { _context.Chats.RemoveRange(existing.Skip(1)); await _context.SaveChangesAsync(); }
                await EnsurePrivateMembersAsync(chat, currentUserId, otherUserId);
                return chat;
            }
            var newChat = new Chat { Type = ChatType.Private, Name = string.Empty, CreatedByUserId = currentUserId, User1Id = currentUserId, User2Id = otherUserId };
    ...
    {
        var ids = new[] { ownerId, otherId };
        foreach (var id in ids)
            if (!await _context.ChatMembers.AnyAsync(m => m.ChatId == chat.Id && m.UserId == id))
                _context.ChatMembers.Add(new ChatMember { ChatId = chat.Id, UserId = id, Role = id == ownerId ? ChatMemberRole.Owner : ChatMemberRole.Member, JoinedAt = chat.CreatedAt });
        await _context.SaveChangesAsync();
    }
    ...
    public async Task<List<Chat>> GetUserChatsAsync(long userId)
    {
        var chats = await _context.Chats.AsNoTracking()
            ...
            .Where(c => c.Members.Any(m => m.UserId == userId) || (c.Type == ChatType.Private && (c.User1Id == userId || c.User2Id == userId)))
            .OrderByDescending(c => c.CreatedAt).ThenByDescending(c => c.Id).ToListAsync();
    ...
    public Task<bool> CanAccessChatAsync(int chatId, long userId) => _context.Chats.AnyAsync(c => c.Id == chatId && (c.Members.Any(m => m.UserId == userId) || (c.Type == ChatType.Private && (c.User1Id == userId || c.User2Id == userId))));
    ...
    public async Task<Message?> SendMessageAsync(int chatId, long senderId, string content)
    {
        ...
        var chat = await _context.Chats.FirstOrDefaultAsync(c => c.Id == chatId); if (chat == null) return null;
    ...
    public async Task<List<ChatMember>> GetMembersAsync(int chatId)
        => await _context.ChatMembers.AsNoTracking().Include(m => m.User).Where(m => m.ChatId == chatId).OrderByDescending(m => m.Role).ThenBy(m => m.JoinedAt).ToListAsync();
    ...
    {
        var chat = await _context.Chats.FirstOrDefaultAsync(c => c.Id == chatId && c.Type == ChatType.Group); if (chat == null) return false;
        var actor = await GetMemberAsync(chatId, actorId); if (actor == null || actor.Role == ChatMemberRole.Member) return false;
        if (await GetMemberAsync(chatId, userId) != null) return true;
        if (!await UserExistsAsync(userId)) return false;
        _context.ChatMembers.Add(new ChatMember { ChatId = chatId, UserId = userId, Role = ChatMemberRole.Member }); await _context.SaveChangesAsync(); return true;
    }

    public async Task<bool> RemoveMemberAsync(int chatId, long actorId, long userId)
    {
        var chat = await _context.Chats.FirstOrDefaultAsync(c => c.Id == chatId && c.Type == ChatType.Group); if (chat == null) return false;
        var actor = await GetMemberAsync(chatId, actorId); var target = await GetMemberAsync(chatId, userId);
        ...
        _context.ChatMembers.Remove(target); await _context.SaveChangesAsync(); return true;
    }

    public async Task<bool> LeaveGroupAsync(int chatId, long userId)
    {
        var member = await GetMemberAsync(chatId, userId); if (member == null || member.Role == ChatMemberRole.Owner) return false;
        _context.ChatMembers.Remove(member); await _context.SaveChangesAsync(); return true;
    }

    public async Task<bool> RenameGroupAsync(int chatId, long actorId, string name)
    {
        name = name.Trim(); if (string.IsNullOrWhiteSpace(name) || name.Length > 128) return false;
        var chat = await _context.Chats.FirstOrDefaultAsync(c => c.Id == chatId && c.Type == ChatType.Group); if (chat == null) return false;
        var actor = await GetMemberAsync(chatId, actorId); if (actor == null || actor.Role == ChatMemberRole.Member) return false;
        chat.Name = name; await _context.SaveChangesAsync(); return true;
    }
