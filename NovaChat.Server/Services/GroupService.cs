...
public class GroupService
{
    ...
    public async Task<List<GroupResponseDto>> GetMyGroupsAsync(string userId) => await _context.GroupMembers
        .AsNoTracking().Where(m => m.UserId == userId)
        .OrderByDescending(m => m.Group.CreatedAt)
        .Select(m => new GroupResponseDto { Id = m.GroupId, Name = m.Group.Name, Description = m.Group.Description, CreatorId = m.Group.CreatorId, CreatedAt = m.Group.CreatedAt })
        .ToListAsync();

    public async Task<GroupResponseDto?> GetAsync(int groupId, string userId)
    {
        if (!await IsMemberAsync(groupId, userId)) return null;
        return await _context.Groups.AsNoTracking().Where(g => g.Id == groupId)
            .Select(g => new GroupResponseDto { Id = g.Id, Name = g.Name, Description = g.Description, CreatorId = g.CreatorId, CreatedAt = g.CreatedAt }).FirstOrDefaultAsync();
    }

    public async Task<List<GroupMemberResponseDto>?> GetMembersAsync(int groupId, string userId)
    {
        if (!await IsMemberAsync(groupId, userId)) return null;
        return await _context.GroupMembers.AsNoTracking().Where(m => m.GroupId == groupId)
            .OrderBy(m => m.Role).ThenBy(m => m.User.DisplayName)
            .Select(m => new GroupMemberResponseDto { UserId = m.UserId, DisplayName = m.User.DisplayName, Role = m.Role.ToString(), JoinedAt = m.JoinedAt }).ToListAsync();
    }
