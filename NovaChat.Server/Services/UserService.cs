...
public class UserService
{
    ...
    public async Task<RegisterResult> RegisterAsync(RegisterDto dto)
    {
        ...
        if (!TryNormalizePhoneNumber(dto.PhoneNumber, out var phoneNumber))
            return Fail("Enter a valid phone number using 7 to 15 digits.");
    ...
    public async Task<User?> LoginAsync(LoginDto dto)
    {
        ...
        // Existing accounts created with the previous PBKDF2 hasher are
        // upgraded automatically after a successful login. Argon2id hashes
        // are also refreshed automatically if their configured work factor
    ...
    public async Task<RegisterResult> UpdateUserAsync(string id, UpdateUserDto dto)
    {
        ...
        if (await _context.Users.AsNoTracking().AnyAsync(u => u.Username == newUsername && u.Id != userId))
            return Fail("This username is already taken.");

        if (await _context.Users.AsNoTracking().AnyAsync(u => u.Email == dto.Email && u.Id != userId))
            return Fail("This Email is already registered.");

        if (await _context.Users.AsNoTracking().AnyAsync(u => u.PhoneNumber == phoneNumber && u.Id != userId))
            return Fail("This phone number is already registered.");
    ...
    public async Task<bool> DeleteUserAsync(string id)
    {
        ...
        try
        {
            ...
            var membersExist = await TableExistsAsync("GroupMembers");
            ...
            {
                await _context.Database.ExecuteSqlInterpolatedAsync($"DELETE FROM `GroupMembers` WHERE `UserId` = {userId}");
            }
            else if (groupsExist)
            {
                await _context.Database.ExecuteSqlInterpolatedAsync($"DELETE FROM `Groups` WHERE `CreatorId` = {userId}");
    ...
    private async Task<bool> TableExistsAsync(string tableName) =>
        await _context.Database
            .SqlQueryRaw<bool>("SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = DATABASE() AND table_name = {0}) AS `Value`", tableName)
            .FirstAsync();
    ...
    {
        Id = user.Id.ToString(System.Globalization.CultureInfo.InvariantCulture),
        ...
        IsOnline = _presenceService.IsOnline(user.Id.ToString(System.Globalization.CultureInfo.InvariantCulture)),
