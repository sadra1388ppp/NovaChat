...
public class UserService
{
    ...
    public async Task<RegisterResult> RegisterAsync(RegisterDto dto)
    {
        ...
        if (existingUserId != null) return new RegisterResult { Success = false, Message = "This User ID is already taken." };
        var existingEmail = await _context.Users.FirstOrDefaultAsync(u => u.Email == dto.Email);
        if (existingEmail != null) return new RegisterResult { Success = false, Message = "This Email is already registered." };
