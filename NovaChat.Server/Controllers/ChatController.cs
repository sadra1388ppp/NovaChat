...
public class ChatController : ControllerBase
{
    ...
    { _chatService = chatService; _configuration = configuration; _hub = hub; _logger = logger; }
    ...
    public async Task<IActionResult> CreateChat(CreateChatDto dto)
    {
        ...
        var otherUser = await _chatService.GetUserByUsernameAsync(username);
        ...
        try
        {
            ...
            var user1 = chat.User1Id.ToString(System.Globalization.CultureInfo.InvariantCulture);
            var user2 = chat.User2Id.ToString(System.Globalization.CultureInfo.InvariantCulture);
            var current = currentUserId.ToString(System.Globalization.CultureInfo.InvariantCulture);
        ...
        {
            _logger.LogError(ex, "Failed to create private chat between {CurrentUserId} and {Username}.", currentUserId, username);
            return Problem(statusCode: 500, title: "Private chat creation failed", detail: "The server could not create the private chat.");
        }
    }
    ...
    public async Task<IActionResult> GetMessages(int chatId, [FromQuery] int? beforeMessageId = null, [FromQuery] int pageSize = 50)
    {
        ...
        return Ok(new ChatHistoryResponseDto { Messages = messages.Select(MessageDtoMapper.Map).ToList(), HasMore = first != null && await _chatService.HasOlderMessagesAsync(chatId, first.Id), NextBeforeMessageId = first?.Id });
    }
    ...
    public async Task<IActionResult> SendMessage(int chatId, SendMessageDto dto)
    {
        ...
        if (chat != null) await _hub.Clients.Users(chat.User1Id.ToString(), chat.User2Id.ToString()).SendAsync("ReceiveMessage", MessageDtoMapper.Map(message));
    ...
    public async Task<IActionResult> DeleteChat(int chatId)
    {
        ...
        await _hub.Clients.Users(chat.User1Id.ToString(), chat.User2Id.ToString()).SendAsync("ChatDeleted", new { chatId = chat.Id, deletedBy = userId.ToString() });
    ...
    public async Task<IActionResult> DeleteMessage(int messageId)
    {
        ...
        await _hub.Clients.Users(chat.User1Id.ToString(), chat.User2Id.ToString()).SendAsync("MessageDeleted", new { id = message.Id, chatId = message.ChatId, senderId = message.SenderId.ToString(), content = message.Content, sentAt = message.SentAt });
        return Ok(new { message = "Message deleted successfully." });
    }

    private ChatListDto MapChat(Chat chat, Message? lastMessage) => new()
    {
        Id = chat.Id, User1Id = chat.User1Id.ToString(), User2Id = chat.User2Id.ToString(),
