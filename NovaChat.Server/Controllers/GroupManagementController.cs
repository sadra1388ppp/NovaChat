...
public class GroupManagementController : ControllerBase
{
    ...
    public async Task<IActionResult> SetMemberRole(int chatId, long userId, [FromBody] SetGroupMemberRoleRequest request)
    {
        ...
        var chat = await _db.Chats.FirstOrDefaultAsync(c => c.Id == chatId && c.Type == ChatType.Group);
        ...
        if (actor == null || actor.Role != ChatMemberRole.Owner) return Forbid();
        if (target == null) return NotFound(new { message = "Member not found." });
        if (target.Role == ChatMemberRole.Owner) return BadRequest(new { message = "The group owner cannot be changed to another role." });

        target.Role = request?.IsAdmin == true ? ChatMemberRole.Admin : ChatMemberRole.Member;
        ...
        return Ok(new { message = target.Role == ChatMemberRole.Admin ? "Member promoted to admin." : "Admin privileges removed." });
    }
    ...
    public async Task<IActionResult> LeaveGroup(int chatId)
    {
        ...
        var chat = await _db.Chats.AsNoTracking().FirstOrDefaultAsync(c => c.Id == chatId && c.Type == ChatType.Group);
        if (chat == null) return NotFound(new { message = "Group not found." });

        var member = await _db.ChatMembers.FirstOrDefaultAsync(m => m.ChatId == chatId && m.UserId == userId);
        if (member == null) return Forbid();
        if (member.Role == ChatMemberRole.Owner) return BadRequest(new { message = "The owner cannot leave the group. Delete the group from Group Info instead." });
    ...
    public async Task<IActionResult> DeleteGroup(int chatId)
    {
        ...
        var owner = await _db.ChatMembers.FirstOrDefaultAsync(m => m.ChatId == chatId && m.UserId == userId && m.Role == ChatMemberRole.Owner);
