...
public class ChatHub : Hub
{
    ...
    {
        var userId = CurrentUserId(); if (string.IsNullOrWhiteSpace(userId)) { Context.Abort(); return; }
        if (_presenceService.UserConnected(userId)) await Clients.All.SendAsync("UserOnline", userId);
        await Clients.Caller.SendAsync("PresenceSnapshot", _presenceService.GetOnlineUsers()); await base.OnConnectedAsync();
    }
    public override async Task OnDisconnectedAsync(Exception? exception)
    {
        var userId = CurrentUserId(); if (!string.IsNullOrWhiteSpace(userId) && _presenceService.UserDisconnected(userId)) { await _userService.MarkLastSeenAsync(userId); await Clients.All.SendAsync("UserOffline", userId); }
    ...
    public async Task SendMessage(int chatId, string content)
    {
        ...
        var message = await _chatService.SendMessageAsync(chatId, userId, content); var chat = await _chatService.GetChatByIdAsync(chatId);
    ...
    public async Task DeleteMessage(int messageId)
    {
        ...
        var chat = await _chatService.GetChatByIdAsync(message.ChatId); if (chat == null || !await _chatService.DeleteMessageAsync(messageId)) throw new HubException("Unable to send deletion.");
        await Clients.Users(Recipients(chat)).SendAsync("MessageDeleted", new { id = message.Id, chatId = message.ChatId, senderId = message.SenderId.ToString(), content = message.Content, sentAt = message.SentAt });
    }
    ...
    private IEnumerable<string> Recipients(Chat chat) => chat.Type == ChatType.Group ? chat.Members.Select(m => m.UserId.ToString()) : new[] { chat.User1Id, chat.User2Id }.Where(x => x.HasValue).Select(x => x!.Value.ToString());
