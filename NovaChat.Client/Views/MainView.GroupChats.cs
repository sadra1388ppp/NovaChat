...
public partial class MainView
{
    ...
    static MainView() { if (_groupUiRegistered) return; _groupUiRegistered = true; EventManager.RegisterClassHandler(typeof(MainView), FrameworkElement.LoadedEvent, new RoutedEventHandler(OnGroupUiLoaded)); }
    ...
    private void InstallGroupUi() { if (_createGroupButton != null) return; if (SearchTextBox.Parent is not Grid searchGrid || searchGrid.Parent is not Border searchBorder || searchBorder.Parent is not StackPanel panel) return; _createGroupButton = new Button { Content = "👥   New group", Height = 40, Margin = new Thickness(0, 6, 0, 0), HorizontalContentAlignment = HorizontalContentAlignment.Left, Padding = new Thickness(10, 0, 10, 0), Style = (Style)FindResource("SecondaryButtonStyle") }; _createGroupButton.Click += CreateGroupButton_Click; panel.Children.Add(_createGroupButton); }
    ...
    private async void CreateGroupButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        createButton.Click += async (_, _) => { var name = nameBox.Text.Trim(); var selected = membersList.Items.OfType<CheckBox>().Where(x => x.IsChecked == true).Select(x => x.Tag as GroupUserSearchModel).Where(x => x != null).Select(x => x!.Username).Distinct(StringComparer.OrdinalIgnoreCase).ToList(); if (string.IsNullOrWhiteSpace(name)) { MessageBox.Show("Enter a group name.", "NovaChat", MessageBoxButton.OK, MessageBoxImage.Warning); return; } if (selected.Count == 0) { MessageBox.Show("Select at least one other member.", "NovaChat", MessageBoxButton.OK, MessageBoxImage.Warning); return; } try { createButton.IsEnabled = false; var result = await _apiService.PostAsync<CreateGroupRequest, CreateGroupResponse>("api/Chat/group", new CreateGroupRequest { Name = name, Usernames = selected }); if (result?.Chat == null) throw new InvalidOperationException("The group could not be created."); dialog.Close(); await LoadChatsAsync(); var item = _chats.FirstOrDefault(x => x.Chat.Id == result.Chat.Id); if (item != null) await OpenChatAsync(item.Chat); } catch (Exception ex) { createButton.IsEnabled = true; MessageBox.Show(ex.Message, "NovaChat", MessageBoxButton.OK, MessageBoxImage.Warning); } };
        dialog.Content = root; dialog.Loaded += (_, _) => nameBox.Focus(); dialog.ShowDialog();
    }

    private async Task RefreshGroupUserSearchAsync(string query, ListBox target, TextBlock countText) { try { target.Items.Clear(); var users = string.IsNullOrWhiteSpace(query) ? [] : await _apiService.GetAsync<List<GroupUserSearchModel>>($"api/User/search?q={Uri.EscapeDataString(query.Trim())}") ?? []; foreach (var user in users) { var check = new CheckBox { Content = $"{user.DisplayName}  •  @{user.Username}", Tag = user, Margin = new Thickness(6, 5, 6, 5), Foreground = (Brush)FindResource("TextBrush") }; check.Checked += (_, _) => countText.Text = $"{target.Items.OfType<CheckBox>().Count(x => x.IsChecked == true)} selected"; check.Unchecked += (_, _) => countText.Text = $"{target.Items.OfType<CheckBox>().Count(x => x.IsChecked == true)} selected"; target.Items.Add(check); } countText.Text = $"{target.Items.OfType<CheckBox>().Count(x => x.IsChecked == true)} selected"; } catch { }
    }

    private void UpdateGroupUserCards() { }
}
