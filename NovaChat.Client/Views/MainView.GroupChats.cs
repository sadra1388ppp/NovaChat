...
public partial class MainView
{
    ...
    private async void CreateGroupButton_Click(object? sender, RoutedEventArgs e)
    {
        ...
        createButton.Click += async (_, _) => { var name = nameBox.Text.Trim(); var selected = membersList.Items.OfType<CheckBox>().Where(x => x.IsChecked == true).Select(x => x.Tag as GroupUserSearchModel).Where(x => x != null).Select(x => x!.Username).Distinct(StringComparer.OrdinalIgnoreCase).ToList(); if (string.IsNullOrWhiteSpace(name)) { MessageBox.Show("Please enter a group name.", "Create Group", MessageBoxButton.OK, MessageBoxImage.Warning); return; } if (selected.Count == 0) { MessageBox.Show("Select at least one member.", "Create Group", MessageBoxButton.OK, MessageBoxImage.Warning); return; } try { var payload = new { Name = name, MemberUsernames = selected }; var created = await _apiService.PostAsync<ChatModel>("api/Chat/group", payload); if (created == null || created.Id <= 0) throw new InvalidOperationException("The server did not return the created group."); dialog.DialogResult = true; await LoadChatsAsync(); await OpenChatAsync(created); } catch (Exception ex) { MessageBox.Show($"Could not create group.\n\n{ex.Message}", "Create Group", MessageBoxButton.OK, MessageBoxImage.Error); } }; searchBox.Focus(); dialog.Content = root; dialog.ShowDialog();
    }
