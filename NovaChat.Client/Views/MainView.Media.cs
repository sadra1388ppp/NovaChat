...
public partial class MainView
{
    ...
    private async Task UploadMediaAsync(string type, string path, double? durationSeconds = null)
    {
        ...
        try
        {
            ...
            if (durationSeconds.HasValue)
                endpoint += $"&durationSeconds={durationSeconds.Value:F2}";
        ...
        if (border.Child is StackPanel old)
        {
            ...
            if (!string.IsNullOrWhiteSpace(time))
            {
                stack.Children.Add(new TextBlock
                {
                    ...
                    Foreground = mine ? Brushes.White : SystemColors.GrayTextBrush,
    ...
    private async Task SaveImageCopyAsync(int messageId, string suggestedName)
    {
        ...
        {
            FileName = string.IsNullOrWhiteSpace(baseName) ? "NovaChatImage" : baseName + ".jpg",
            Filter = "Image|*.jpg;*.png;*.webp;*.gif|All files|*.*",
            Title = "Save image"
        };
        if (dialog.ShowDialog() != true) return;
    ...
    private async Task DownloadMediaAsync(int messageId, string suggestedName)
    {
        ...
        var safeName = suggestedName.Replace("📎 ", string.Empty).Replace("📷 ", string.Empty).Replace("🎙 ", string.Empty);
        var dialog = new SaveFileDialog { FileName = safeName, Title = "Save file" };
        if (dialog.ShowDialog() != true) return;
