...
public partial class ProfileView : UserControl
{
    ...
    private async Task UpdateProfileUiAsync()
    {
        ...
        {
            var avatarEndpoint = _apiService.BuildAbsoluteUrl(
                $"api/User/profile/{Uri.EscapeDataString(_profile.Id)}/avatar?avatar={Uri.EscapeDataString(_profile.AvatarUrl)}");
    ...
    private async void ChangePictureButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            var result = await _apiService.UploadFileAsync<ProfileActionResponse>(
                $"api/User/{Uri.EscapeDataString(AuthState.UserId)}/avatar", dialog.FileName);
        ...
        save.Click += async (_, _) =>
        {
            ...
            try
            {
                var response = await _apiService.PutAsync<ChangePasswordRequest, SimpleMessageResponse>(
                    ...
                    new ChangePasswordRequest { CurrentPassword = currentBox.Password, NewPassword = newBox.Password });

                feedback.Text = response?.Message ?? "Password updated.";
